...
Future<void> showPhotoActionSheet(BuildContext context) async {
  int choice = 0;
  bool hasRemoveImageButton =
      context.read<MemberCubit>().state.profileImage == null;

  final actions = [
    hasRemoveImageButton
        ? Container()
        : BottomSheetAction(
      onPressed: (_) {
        choice = 1;
      ...
      title: Text(
        ...
        style: TextStyle(color: KColors.fieldBorderError),
      ),
    ),
    BottomSheetAction(
      onPressed: (_) {
    ...
    if (img.lengthInBytes > _maxImageSizeInBytes) {
      AppToast.instance.showError(
          ...
          AppLocalizations.of(context)!
              .max_image_size_is(_maxImageSizeInBytes ~/ 1000));
      return;
    }
